...
export function SettingsPage(){
 ...
 return <>
  ...
  <div className="settings-layout"><SectionCard className="settings-nav">{['Allgemein','DICOM','Logging','Retention','Datensicherung','UI'].map(item=><button type="button" className={section===item?'active':''} aria-pressed={section===item} onClick={()=>{setSection(item);setNotice(undefined)}} key={item}>{item}</button>)}</SectionCard><SectionCard title={section}><form className="settings-panel" onSubmit={submit}>
   {section==='DICOM'&&<><FormField label="Default Calling AE" htmlFor="setting-ae"><TextInput id="setting-ae" className="mono" maxLength={16} pattern="[A-Za-z0-9 _.-]{1,16}" required value={value.callingAe} onChange={e=>setValue({...value,callingAe:e.target.value.toUpperCase()})}/></FormField><FormField label="Connect Timeout (Sekunden)" htmlFor="connect-timeout"><NumberInput id="connect-timeout" min={1} max={120} value={value.connectTimeout} onChange={e=>setValue({...value,connectTimeout:Number(e.target.value)})}/></FormField><FormField label="Association Timeout (Sekunden)" htmlFor="association-timeout"><NumberInput id="association-timeout" min={1} max={120} value={value.associationTimeout} onChange={e=>setValue({...value,associationTimeout:Number(e.target.value)})}/></FormField><FormField label="DIMSE Timeout (Sekunden)" htmlFor="dimse-timeout"><NumberInput id="dimse-timeout" min={1} max={300} value={value.dimseTimeout} onChange={e=>setValue({...value,dimseTimeout:Number(e.target.value)})}/></FormField><AlertBox tone="info">Das Default Calling AE wird für neue manuelle Tests, Ziele und Modalitätsprofile verwendet. Serverseitige Timeouts werden weiterhin über die lokale Container-Konfiguration gesetzt.</AlertBox></>}
   {section==='Logging'&&<><FormField label="Log Level" htmlFor="log-level"><Select id="log-level" value={value.logLevel} onChange={e=>setValue({...value,logLevel:e.target.value})}>{['DEBUG','INFO','WARNING','ERROR'].map(item=><option key={item}>{item}</option>)}</Select></FormField><AlertBox tone="info">Das serverseitige Log Level wird über die lokale Container-Konfiguration gesetzt. Patientendatensätze werden nicht automatisch in das Application Log geschrieben.</AlertBox></>}
