...
export function Dashboard(){
 ...
 return <>
  ...
  <div className="metric-grid"><MetricCard label="Aktive Ziele" value={allTargets.length} caption="konfigurierte Endpunkte" icon={<Target size={26}/>}/><MetricCard label="Tests heute" value={summary?summary.today_total:'—'} caption={summary?`${summary.today_success} erfolgreich`:'Kennzahl nicht verfügbar'} icon={<Activity size={26}/>} tone="navy"/><MetricCard label="Erfolgsquote" value={summary?`${success} %`:'—'} caption={summary?(summary.today_total?'für heutige Tests':'noch keine Tests'):'Kennzahl nicht verfügbar'} icon={<CheckCircle2 size={27}/>} tone="teal"/><MetricCard label="Letzte Antwortzeit" value={last?`${last.duration_ms} ms`:'—'} caption={last?typeLabel(last.test_type):'noch kein Ergebnis'} icon={<Clock3 size={26}/>} tone="blue"/></div>
  ...
  <div className="dashboard-grid"><SectionCard title="Zielstatus" action={<Link to="/targets">Alle anzeigen →</Link>}>{targets.isLoading?<LoadingState/>:<DataTable columns={targetColumns} rows={allTargets.slice(0,5)} getKey={row=>row.id} empty="Noch keine Ziele konfiguriert."/>}</SectionCard><SectionCard title="Letzte Tests" action={<Link to="/history">Alle anzeigen →</Link>}>{runs.isLoading?<LoadingState/>:<DataTable columns={runColumns} rows={allRuns.slice(0,6)} getKey={row=>row.id} onRowClick={row=>location.assign(`/history/${row.id}`)} empty="Noch keine Tests durchgeführt."/>}</SectionCard></div>
  <div className="bottom-grid"><SectionCard title="Testverteilung"><div className="activity-list"><div className="activity-item"><StatusDot tone="info"/><div><strong>Worklist</strong><small>{summary?.by_type.mwl_find??'—'} Tests</small></div></div><div className="activity-item"><StatusDot tone="success"/><div><strong>PACS Store</strong><small>{summary?.by_type.dicom_store??'—'} Tests</small></div></div><div className="activity-item"><StatusDot tone="info"/><div><strong>PACS-Suche</strong><small>{summary?.by_type.qr_find??'—'} Tests</small></div></div><div className="activity-item"><StatusDot tone="success"/><div><strong>Modalitätsprüfung</strong><small>{summary?.by_type.modality_check??'—'} Tests</small></div></div><div className="activity-item"><StatusDot tone="info"/><div><strong>C-ECHO</strong><small>{summary?.by_type.dicom_echo??'—'} Tests</small></div></div></div></SectionCard><SectionCard title="Konfigurierte Dienste"><div className="activity-list"><div className="activity-item"><StatusDot tone="success"/><div><strong>Worklist-Ziele</strong><small>{allTargets.filter(t=>t.mwl_enabled).length} aktiv</small></div></div><div className="activity-item"><StatusDot tone="info"/><div><strong>Store-Ziele</strong><small>{allTargets.filter(t=>t.store_enabled).length} aktiv</small></div></div><div className="activity-item"><StatusDot tone="info"/><div><strong>Query/Retrieve-Ziele</strong><small>{allTargets.filter(t=>t.qr_enabled).length} aktiv</small></div></div></div></SectionCard><SectionCard title="Hinweise"><div className="activity-list">{allRuns.slice(0,3).map(run=><div className="activity-item" key={run.id}><StatusDot tone={run.success?'success':'error'}/><div><strong>{typeLabel(run.test_type)} {run.success?'erfolgreich':'fehlgeschlagen'}</strong><small>{new Date(run.started_at).toLocaleString('de-DE')}</small></div></div>)}{!allRuns.length&&<div className="activity-item"><StatusDot tone="info"/><div><strong>DCMSim ist bereit</strong><small>Ersten Test starten</small></div></div>}</div></SectionCard></div>
 </>
}
