...
`apps/api` enthält FastAPI, SQLAlchemy, Alembic und DICOM-Domänenlogik. `apps/web` enthält React/Vite. `apps/web/e2e` enthält kontrollierte Browser-Flows, `apps/web/e2e-full-stack` die echten DICOM-Smokes und `docs` die Betriebs- und Architekturunterlagen.
...
Anwenden: `alembic -c apps/api/alembic.ini upgrade head`. Erstellen: `alembic -c apps/api/alembic.ini revision --autogenerate -m "kurze beschreibung"`. Die generierte Migration gegen eine leere SQLite-Datei und bei Datenmigrationen zusätzlich gegen das direkte Vorgängerschema testen. Vor `0006` ein Backup anlegen; die Migration normalisiert Legacy-Ziele, löscht sie aber nicht.
...
- Full Stack: `npm run e2e:full-stack` startet FastAPI, temporäres SQLite, React sowie lokale MWL-, Study-Root- und Storage-SCPs und darf nicht durch API-Mocks ersetzt werden
...
Version in `pyproject.toml`, FastAPI-Metadaten, `package.json`, `package-lock.json` und allen sichtbaren Anzeigen gemeinsam aktualisieren; Changelog ergänzen; Backend-, Frontend-, E2E-, Migration- und Docker-Prüfungen ausführen; DICOM Support Matrix, User Manual und ADRs abgleichen; anschließend Tag gemäß Semantic Versioning erstellen. Vor einem Upgrade empfiehlt sich ein SQLite-Backup über **Einstellungen → Datensicherung**.
