...
Bei Docker zeigt ein DICOM-Ziel `127.0.0.1` auf den DCMSim-Container selbst. Für ein PACS im Netzwerk dessen vom Container erreichbare IP oder DNS-Adresse verwenden. Die lokale Bindung der Weboberfläche an `127.0.0.1:8080` blockiert ausgehendes DICOM nicht. Zusätzlich Quell-IP, Firewall und AE-Freigabe der Gegenstelle prüfen.
